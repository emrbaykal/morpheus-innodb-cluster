...
<!-- 📸 INSERT IMAGE: HPE Morpheus 3-Node HA Architecture Diagram -->
<!-- If you have the architecture diagram from the HPE documentation (page 3 of the 3-Node HA Install PDF), insert it here. It shows "3 Node HA with Distributed Database" with App Nodes, Database Cluster, Load Balancer, and Shared Storage. -->
...
<!-- 📸 INSERT SCREENSHOT 1: environment-setup.png -->
<!-- The screenshot showing "MySQL InnoDB Cluster Setup Orchestrator" banner, "Step 1/11 ENVIRONMENT SETUP", OS detection, package installation, and "Environment ready. (ansible [core 2.15.13])" message -->
...
<!-- 📸 INSERT SCREENSHOT 2: cluster-configuration-nodes-ssh.png -->
<!-- The screenshot showing "Step 2/11 CLUSTER CONFIGURATION" with Section 1/5 (three node entries with hostnames and IPs) and Section 2/5 (SSH user, password, privilege escalation) -->
...
<!-- 📸 INSERT SCREENSHOT 3: mysql-credentials-cluster-ntp.png -->
<!-- The screenshot showing Section 3/5 (MySQL Credentials), Section 4/5 (Cluster Settings with "morpheus-cluster"), and Section 5/5 (System Settings with NTP servers) -->
...
<!-- 📸 INSERT SCREENSHOT 4: configuration-summary.png -->
<!-- The screenshot showing the "CONFIGURATION SUMMARY" table with CLUSTER NODES, SSH CONNECTION, MYSQL CONFIGURATION, and SYSTEM SETTINGS sections. Shows "Is this configuration correct? [Y/n]: y" and "Configuration saved to: /root/morpheus-innodb-cluster/cluster_config.json" -->
...
<!-- 📸 INSERT SCREENSHOT 5: inventory-ssh-test.png -->
<!-- The screenshot showing "Step 3/11 GENERATING INVENTORY" (with the full inventory.ini content including [mysql_nodes] and [mysql_nodes:vars]) and "Step 4/11 SSH CONNECTIVITY TEST" (with all three nodes showing OK) -->
...
<!-- 📸 INSERT SCREENSHOT 6: preflight-checks.png -->
<!-- The screenshot showing Steps 5/11 (RHEL REPOSITORY CHECK with all repos enabled), 6/11 (PRE-FLIGHT MYSQL PACKAGE CHECK with all "not installed"), and 7/11 (INTERNET CONNECTIVITY CHECK with all nodes showing "dev.mysql.com OK") -->
...
<!-- 📸 INSERT SCREENSHOT 7: mysql-version-selection.png -->
<!-- The screenshot showing "Step 8/11 MYSQL VERSION SELECTION" with AppStream stream choices (1. 8.0, 2. 8.4), selection of 8.4, then version choices (8.4.4, 8.4.6, 8.4.7), and "MySQL 8.4.7 (stream: 8.4) selected" -->
...
<!-- 📸 INSERT SCREENSHOT 8: deployment-confirmation.png -->
<!-- The screenshot showing "Step 9/11 DEPLOYMENT CONFIRMATION" with cluster name, admin user, master node (starred), two slave nodes (bulleted), and the yellow text "This will install MySQL, configure InnoDB Cluster, create the router user, and verify cluster status." -->
...
<!-- 📸 INSERT SCREENSHOT 9: ansible-playbook-execution.png -->
<!-- The screenshot showing "Step 10/11 RUNNING ANSIBLE PLAYBOOK" with cluster/master/playbook info, start time, and the ANSIBLE OUTPUT showing PLAY [all], TASK [Gathering Facts], TASK [Populate /etc/hosts], TASK [Set hostname], etc. -->
...
<!-- 📸 INSERT SCREENSHOT 10: setup-report-header.png -->
<!-- The screenshot showing "Step 11/11 SETUP REPORT" with the "MySQL InnoDB Cluster Setup Report" banner, Report Date, Total Duration (4 min 31 sec), Overall Status (SUCCESS), Ansible Exit Code (0), RHEL REPOSITORY STATUS section, and PRE-EXISTING MYSQL PACKAGES table -->
...
<!-- 📸 INSERT SCREENSHOT 11: node-status-next-steps.png -->
<!-- The screenshot showing NODE STATUS table (innodb-rhel-1: 70 OK, 32 Changed, 0 Unreachable, 0 Failed; innodb-rhel-2 and innodb-rhel-3: 61 OK, 26 Changed each), APPLIED ROLES, NEXT STEPS with mysqlsh commands, and FILE LOCATIONS -->
